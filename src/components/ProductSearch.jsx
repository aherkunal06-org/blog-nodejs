...
const ProductSearch = ({
    ...
    const search = async () => {
      ...
      try {
        const [productsData, blogsData] = await Promise.all([
          productsApi.getAll({ status: 'active', limit: 5 }).catch(() => ({ products: [] })),
          blogsApi.getAll({ limit: 5, search: query }).catch(() => ({ blogs: [] })),
        ]);
  ...
  const allItems = useMemo(() => {
    ...
    results.blogs.forEach((blog, idx) => {
      items.push({ type: 'blog', data: blog, index: results.products.length + idx });
    });
    return items;
  }, [results]);
