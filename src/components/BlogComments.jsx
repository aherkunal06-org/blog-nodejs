...
export default function BlogComments({ blogId }) {
  ...
  const session = getUserInfo(); // Get user info from JWT
  ...
  const fetchComments = async () => {
    try {
      setLoading(true);
      const res = await apiClient.get(`/comments?blogId=${blogId}&page=${currentPage}&limit=10`);
      if (res.data.success) {
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {comment.user.email && session?.email === comment.user.email && (
